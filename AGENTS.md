...
- `js/main.js` — données + rendu : navigation, carrousel (tableau `heroSlides`), offres web (tableau `offers`), réalisations (tableau `projects`), témoignages, investissement et footer sont générés en JS depuis les tableaux. « Modifier le contenu = éditer les données de `js/main.js` », pas `index.html` (sections injectées par les fonctions `render*` dans des conteneurs vides). `index.html` est fixe.
- Sections de la page (dans l'ordre) : héros (carrousel 3D), marquee, bande image `#hero-band`, `#a-propos` (piliers + chiffres), `#offres` (offres + bandeau promo + brochure), `#realisations` (carrousels Bootstrap, liens de visite), `#investissement`, `#temoignages`, `#contact`, footer. La section « Services » a été supprimée sur demande (aucun `#services` dans la nav).
...
- Conserver la trajectoire des classes générées : `renderOffers` → `.offer-card/.offer-price/.offer-badge(-hot)/.offer-cta` ; `renderProjects` → `.project-card` + `icon-box soft-*` (et `.project-media` = carrousel Bootstrap quand `images` est présent, badge projet sur la diapositive) ; carrousel → chaque diapositive `.c3d-slide` reçoit `data-href` (navigation au clic) et est redessinée par le moteur JS (rotateY/translateZ, auto-avance 4,2 s, pause au survol).
